...
public interface NoteRepository extends JpaRepository<Note, Long> {
    ...
    @Query("SELECT note FROM Note note where note.title like %:query%")
    List<Note> searchAtTitle(String query);

    @Query("SELECT note FROM Note note where note.content like %:query%")
    List<Note> searchAtContent(String query);
